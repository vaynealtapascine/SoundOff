...
- Increments since the v0.1 slice: reviewed fixes from `agent/final-review`, project schema 2 with a backed-up upgrade, persistent redo, structural paragraph/speaker edits, per-paragraph speaker reassignment, and saved appearance settings.
...
| `dotnet test SoundOff.sln -c Release --no-build --no-restore --logger "trx;LogFileName=SoundOff.Tests.trx" --results-directory artifacts/test-results` | **101 passed, 0 failed, 0 skipped, 101 total**; exit 0. The previous TRX is deleted before the run and the fresh TRX's summary, counters and per-test outcomes are cross-checked by the script. |
...
| Native smoke inside the verification script | Observed the visible native Windows window titled `SoundOff — private fixture editor`, requested WM_CLOSE only for that process, observed clean **exit 0**. Launching created no `%LOCALAPPDATA%\SoundOff` directory: settings are written only when an appearance choice changes. |
...
- **Domain:** Unicode/combining marks/emoji round trips, stable IDs, nullable timing and synthetic overlap, validation limits, frozen export.
...
- **Storage:** transaction rollback and abrupt process exit, durable undo and redo across reopen, redo cleared by a new edit, writer ownership across processes, protected stale revisions, refusal of unsupported/incomplete schemas, and the schema-1 upgrade: a flushed backup precedes the transactional upgrade, an injected interruption before commit leaves the file at schema 1, and the backup is a readable schema-1 copy of the pre-upgrade document.
...
- **Settings:** strict parsing of the appearance file (missing, empty, malformed, missing/unknown/duplicate fields, wrong version/theme/type, oversized, invalid UTF-8), defaults with a visible reason, atomic replacement on the next change, and a reported save failure that still applies the choice in the window.
- **Headless UI:** demo/edit/save/copy/export/reopen/undo/redo through real controls, immediate copy/close after input, invalid drafts, draft export labels, project/export extension policy, theme and reduced-motion effects on template parts, timed-project labels, upgrade notice, paragraph actions (split/merge/insert/delete/add) with undo, edge and surrogate split offsets failing visibly while keeping the draft, speaker add/reassign/remove, and settings persistence across windows.
