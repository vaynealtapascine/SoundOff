...
| `dotnet test SoundOff.sln -c Release --no-build --no-restore --logger "trx;LogFileName=SoundOff.Tests.trx" --results-directory artifacts/test-results` | **125 passed, 0 failed, 0 skipped, 125 total**; exit 0. The previous TRX is deleted before the run and the fresh TRX's summary, counters and per-test outcomes are cross-checked by the script. |
