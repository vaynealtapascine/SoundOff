...
The window opens on a start screen (import, open, try the demo, recent projects). With a project open, the transcript fills the middle; **Project** and **Export** menus, Undo/Redo/Save/Discard and **Find** sit in the top bar; **Audio**, **Transcribe**, **Speakers** and **History** sit in the side panel; and playback runs along the bottom. Explanations live in the in-app **Help** (F1), which renders [docs/HELP.md](docs/HELP.md) bundled into the build, rather than on screen.

## Transcribe a recording

1. **Import audio or video…** (start screen, or **Import…** in the Audio panel) probes the file, creates the project if there is not one yet, and copies the recording into `<project>.soundoff.media/media/`. The transport bar appears once it loads.
...
Shortcuts: **Ctrl+S** save, **Ctrl+Z** undo, **Ctrl+Y** redo, **Ctrl+F** find, **F3** find next, **Esc** close find, **F1** help. Each only triggers the corresponding enabled button, so undo never silently discards typed text.
