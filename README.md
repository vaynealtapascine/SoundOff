...
| Review | Playback lights the word being spoken inside the text itself, from one authoritative clock; double-clicking a word seeks to it. A video's picture decodes locally, windowed a few seconds ahead, and tracks the same clock. |
...
The window opens on a start screen (import, open, try the demo, recent projects). With a project open, the transcript fills the middle. The top bar carries the project's name and its menu at the left, then Undo, Redo and Save; the **Document / Timings** switch sits in the centre; Find, Export, the side panel, Help and Settings are at the right. **Audio**, **Transcribe**, **Speakers** and **History** sit in the side panel; the waveform and playback run along the bottom. The side panel and the waveform are each dragged to size, and the panel button (Ctrl+B) hides the panel to give the transcript the whole window; the sizes, the panel and the chosen view are all remembered. A dot in the status bar carries saved / unsaved / working / failed, and a failure also colours the message beside it. Explanations live in the in-app **Help** (F1), which renders [docs/HELP.md](docs/HELP.md) bundled into the build, rather than on screen.
...
**Views:** one transcript, two readings, switched from the top bar or with Ctrl+1/Ctrl+2. **Document** is a page of words: the speaker's name is printed once over the run of paragraphs they own, the paragraph being spoken carries a rule down its left edge, the word being spoken is highlighted inside the text, and each timed paragraph's start sits in the page's left margin. **Timings** is a cue table in the shape subtitle editors use — number, start, end, length, speaker, text — under headings built from the same column template as the rows, so the two cannot drift apart. Folding a row hides only its editable text and shows one line of it; its number, times and speaker stay, which is what makes the table scannable. Both views share every input control, so switching cannot save, reparse or discard a draft, and it preserves the waveform zoom.
...
**Structure:** each paragraph's **⋯** menu, at the right of the paragraph in both views, offers Split at cursor (also Ctrl+Enter), Merge with next, Insert paragraph below and Delete paragraph; **Add paragraph** sits under the last paragraph. **Add speaker** and the per-speaker **⋯** menu with **Split speaker…**, **Merge into…** and **Remove** (unused speakers only) are in the Speakers panel. Each saves the current draft together with its change as one undoable revision. Split offsets must fall between whole user-perceived characters, so surrogate pairs, combining marks, ZWJ emoji and flag pairs cannot be torn apart.
...
**Find** (Ctrl+F) floats a find bar over the document rather than pushing it down. It searches paragraphs with plain case-insensitive matching and no Unicode normalization. Replacements change only the draft, so Save commits them and Discard reverts them.

Shortcuts: **Ctrl+S** save, **Ctrl+Z** undo, **Ctrl+Y** redo, **Ctrl+F** find, **F3** find next, **Esc** close find, **Ctrl+B** side panel, **Ctrl+1**/**Ctrl+2** the two views, **Ctrl+Space** play/pause, **Alt+Left**/**Alt+Right** five seconds either way, **F8**/**F9** mark this paragraph's start/end at the playhead, **Ctrl+Enter** split at the cursor, **F1** help. Each only triggers the corresponding enabled action, so undo never silently discards typed text; F8 and F9 write into the timing boxes as an ordinary draft edit, which Save keeps and Discard takes back.

**Settings** holds what is set once and rarely changed: appearance (Follow system, Light, Dark), reduced motion (default on, since OS detection is not implemented), the spoken language and processing device for transcription, what to record and on which device, and the video preview's height. **Dark is the default** for a new install or an unreadable settings file; a valid saved choice is never overridden. Both are saved to `%LOCALAPPDATA%\SoundOff\settings.json` (or `SOUNDOFF_SETTINGS_PATH`, see [docs/SETTINGS-ISOLATION.md](docs/SETTINGS-ISOLATION.md)); an invalid file yields defaults with a visible reason rather than a crash.
