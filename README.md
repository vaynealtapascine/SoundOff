...
2. Edit speaker names, whole transcript paragraphs, or the speaker assigned to a paragraph (the selector above each paragraph). Stable speaker/block IDs do not change. Typing remains an **UNSAVED DRAFT**; this slice does **not** claim keystroke autosave.
...
6. **Open project** reopens the SQLite project without a worker, model or media file. A second editor is refused while another SoundOff process owns the project writer lock. A schema-1 project from the earlier v0.1 build is upgraded in place after an untouched backup copy is written beside it; the status line names that backup.
...
- Full-document/word-anchored editing, autosave, library, search, reprocessing comparisons, durable inference jobs, progress/ETA, cancellation checkpoints, tray/notifications, OS reduced-motion detection and migrations beyond the schema 1 to 2 upgrade.
