...
The window opens on a start screen (import, open, try the demo, recent projects). With a project open, the transcript fills the middle; **Project** and **Export** menus, Undo/Redo/Save/Discard and **Find** sit in the top bar; **Audio**, **Transcribe**, **Speakers** and **History** sit in the side panel; and playback runs along the bottom. **Panel** (Ctrl+B) hides the side panel and gives the transcript the whole window; the choice is remembered. A dot in the status bar carries saved / unsaved / working / failed. Explanations live in the in-app **Help** (F1), which renders [docs/HELP.md](docs/HELP.md) bundled into the build, rather than on screen.

Both appearances are hand-built token palettes rather than Fluent's system brushes, and every foreground/background pair in them is at or above WCAG AA 4.5:1 — including the primary button's label, whose colour is a theme token because white on the dark accent would not be.
...
- **Export → Text (.txt)** / **Export → Copy text** — UTF-8 with speaker labels, revision, provenance, and exact `[start – end]` timecodes on paragraphs that carry timing. Never a placeholder for untimed ones. A clearly labelled unsaved draft can be rescued even when it cannot be saved.
