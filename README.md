...
| Review | Playback lights the word being spoken inside the text itself, from one authoritative clock; clicking a word puts the cursor and the playhead there together. A video's picture decodes locally, windowed a few seconds ahead, and tracks the same clock. |
...
**Playback:** play/pause (Ctrl+Space), five-second skips (F8/F9), a position slider, volume and elapsed/duration. The active paragraph highlights, and overlapping speakers both stay highlighted rather than being flattened into one turn.
...
Shortcuts: **Ctrl+S** save, **Ctrl+Z** undo, **Ctrl+Y** redo, **Ctrl+F** find, **F3** find next, **Esc** close find, **Ctrl+B** side panel, **Ctrl+1**/**Ctrl+2** the two views, **Ctrl+Space** play/pause, **Alt+Left**/**Alt+Right** mark this paragraph's start/end at the playhead, **F8**/**F9** five seconds either way, **Ctrl+Enter** split at the cursor, **F1** help. Each only triggers the corresponding enabled action, so undo never silently discards typed text; Alt+Left and Alt+Right write into the timing boxes as an ordinary draft edit, which Save keeps and Discard takes back.

**Settings** holds what is set once and rarely changed: appearance (Follow system, Light, Dark), reduced motion (default on, since OS detection is not implemented), whether the document fills the window, the spoken language and processing device for transcription, and the video preview's height. What to record and on which device stay in the Audio panel, where they are chosen before a take rather than once. **Dark is the default** for a new install or an unreadable settings file; a valid saved choice is never overridden. Both are saved to `%LOCALAPPDATA%\SoundOff\settings.json` (or `SOUNDOFF_SETTINGS_PATH`, see [docs/SETTINGS-ISOLATION.md](docs/SETTINGS-ISOLATION.md)); an invalid file yields defaults with a visible reason rather than a crash.
